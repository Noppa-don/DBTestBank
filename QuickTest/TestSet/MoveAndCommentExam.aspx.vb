...
Public Class MoveAndCommentExam
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Request.QueryString("QSetId") IsNot Nothing AndAlso Request.QueryString("qid") IsNot Nothing Then
            ...
            If Not Page.IsPostBack Then
                'get now Book 
                'SetNowBook(Request.QueryString("qid"))
            End If
...
#Region "Section-Render"
    ...
    Private Sub SetNowBook(QuestionId As String)
        'Check ว่ามีที่ tblChangeBook หรือยัง ถ้ามีแล้วให้ เอา Book QCate QSet มาแสดง ถ้ายังไม่มีให้เอา Id จาก Table จริงมาแสดง
        Dim sql As String
        sql = "select QCategoryId as QCategory_Id,QSetId as QSet_Id,BookGroupId as Book_Id from tblChangeBook where QuestionId = '" & QuestionId & "' and isactive = '1';"
        Dim dt As DataTable
        dt = _DB.getdata(sql)

        If dt.Rows.Count = 0 Then
            sql = "select q.Question_Id,qs.QSet_Id,qc.QCategory_Id,qc.Book_Id
        ...
        If dt.Rows.Count() <> 0 Then
            ...
            sql = "select distinct BookGroup_Id,Book_Name from tblbook where GroupSubject_Id = 'A4B9F5CB-2D3C-4F6A-8666-FD2620E69723' and Level_Id in('6736D029-6B78-4570-9DBB-991217DA8FEE','2E0FFC04-BCEE-45BE-9C0C-B40742523F43','6BF52DC7-314C-40ED-B7F3-BCC87F724880') and IsActive = 1 and Book_Syllabus = '51' order by Book_Name"
            Dim dtBook As DataTable
            dtBook = _DB.getdata(sql)
            ddlBook.DataValueField = "BookGroup_Id"
            ddlBook.DataTextField = "Book_Name"
            ddlBook.DataSource = dtBook
            ddlBook.DataBind()
            ddlBook.SelectedValue = dt.Rows(0)("Book_Id").ToString

            BindQuestionCategory(dt.Rows(0)("Book_Id").ToString, dt.Rows(0)("QCategory_Id").ToString)
            BindQuestionSet(dt.Rows(0)("QCategory_Id").ToString, dt.Rows(0)("QSet_Id").ToString)
...
#Region "Section-GetData"
    ...
    Private Function GetDTAnswerByType(ByVal QuestionId As String, ByVal QsetType As Integer, ByVal QsetId As String, ByRef InputConn As SqlConnection) As DataTable
        ...
        If QsetType = 1 Or QsetType = 2 Then
            sql = " SELECT Answer_Id,Answer_Name_Quiz,Answer_Expain_Quiz FROM dbo.tblAnswer WHERE Question_Id = '" & QuestionId & "' AND IsActive = 1 " &
                  " ORDER BY Answer_No; "
    ...
    Private Sub GenStrAnswerType1(ByVal QsetId As String, ByVal Dt As DataTable, ByRef SbAnswerName As StringBuilder, ByRef SbAnswerExplain As StringBuilder)
        ...
        For i As Integer = 0 To Dt.Rows.Count - 1
            ...
            If i Mod 2 = 0 Then
                ...
                SbAnswerName.Append(PrefixChoice(i) & ".</td><td style='height: 50px;width:45%;'><div answerid='" & AnswerId & "' class='CanEdit EditAnswer'>" & AnswerName & "</div></td> ")
            Else
                'Answer ฝั่งขวา
            ...
            SbAnswerExplain.Append("<div class='CanEdit EditAnswerExplain' answerid='" & AnswerId & "'>")
    ...
    Private Sub GenStrAnswerType2(ByVal Dt As DataTable, ByVal QsetId As String, ByRef SbAnswer As StringBuilder, ByRef SbAnswerExplain As StringBuilder)
        ...
        For index = 0 To Dt.Rows.Count - 1
            ...
            SbAnswerExplain.Append("<div class='CanEdit EditAnswerExplain' answerid='" & AnswerId & "' style='margin-left:0px;'>")
            SbAnswerExplain.Append(AnswerExplain)
            SbAnswerExplain.Append("</div></div></div>")
        Next
    End Sub
    ...
    Private Sub GenStrAnswerType3(ByVal Dt As DataTable, ByVal QsetId As String, ByRef SbAnswerName As StringBuilder, ByRef SbAnswerExplain As StringBuilder)
        ...
        For i = 0 To Dt.Rows.Count - 1
            ...
            SbAnswerName.Append("<tr style=""""><td style=""width:45%;border-bottom:1px solid Gray;padding-right:10px;""><div questionid='" & QuestionId & "' class='CanEdit EditQuestionType3'>" & QuestionName & "</div></td>")
            ...
            SbAnswerName.Append("style=""width:45%;border-bottom:1px solid Gray;padding-left:10px;""><div class='CanEdit EditAnswer' answerid='" & AnswerId & "'  id=""" & AnswerId & """ >" & AnswerName & "</div></td></tr>")
            'AnswerExplain
            ...
            SbAnswerExplain.Append(AnswerName)
            SbAnswerExplain.Append("</td></tr></table><div class='CanEdit EditAnswerExplain' answerid='" & AnswerId & "'  style='margin-left: 0px;'>")
            SbAnswerExplain.Append(AnswerExplain)
            SbAnswerExplain.Append("</div></div></div>")
        Next
    ...
    Private Sub GenStrAnswerType6(ByVal Dt As DataTable, ByVal QsetId As String, ByRef SbAnswer As StringBuilder, ByRef SbAnswerExplain As StringBuilder)
        ...
        For i = 0 To Dt.Rows.Count - 1
            ...
            End If
            'Answer
            SbAnswer.Append("<li><div class='CanEdit EditQuestionType6' questionid='" & QuestionId & "' id=""" & QuestionId & """ >" & QuestionName & "</div></li>")
            'AnswerExplain
            SbAnswerExplain.Append("<li>")
            SbAnswerExplain.Append(QuestionName)
            SbAnswerExplain.Append("<div class='CanEdit EditAnswerExplainType6' answerid='" & AnswerId & "' style='display:block;border: 1px dashed;border-width: 2px;border-radius: 5px;margin-right: 10px;padding: 10px;'>")
            SbAnswerExplain.Append(AnswerExplain)
...
#Region "Section-Save"
    ...
    Public Shared Function SaveAfterEdit(ByVal QuestionSet As String, ByVal AnswerSet As String, ByVal AnswerExplainSet As String) As String
        ...
        Try
            ...
            If dtOld.Rows(0)("Question_Name_Quiz") <> _DB.CleanString(QuestionNameAfterGenPath) Then
                Log.Record(Log.LogType.ManageExam, "แก้ไขคำถามเป็น """ & _DB.CleanString(QuestionNameAfterGenPath) & """(QuestionId=(" & ObjQuestionSet(0).ToString().Trim() & ")", True)
                UpdateStatus = 1
            End If
            If dtOld.Rows(0)("Question_Expain_Quiz") <> _DB.CleanString(QuestionExplainAfterGenPath) Then
                Log.Record(Log.LogType.ManageExam, "แก้ไขคำอธิบายคำถามเป็น """ & _DB.CleanString(QuestionExplainAfterGenPath) & """(QuestionId=(" & ObjQuestionSet(0).ToString().Trim() & ")", True)
                UpdateStatus = 1
            End If
            ...
            If AnswerSet <> "" Then
                ...
                For i As Integer = 0 To ObjMainAnswerSet.Length - 1
                    ...
                    If AnswerOld.Rows(0)("Answer_Name_Quiz") <> _DB.CleanString(AnswerName) Then
                        Log.Record(Log.LogType.ManageExam, "แก้ไขคำตอบเป็น """ & _DB.CleanString(AnswerName) & """(QuestionId=(" & ObjAnswerSet(0).ToString().Trim() & ")", True)
                        UpdateStatus = 1
                    End If
                Next
            End If
            ...
            If AnswerExplainSet <> "" Then
                ...
                For i As Integer = 0 To ObjMainAnswerExplainSet.Length - 1
                    ...
                    If AnswerExpainOld.Rows(0)("Answer_Expain_Quiz") <> _DB.CleanString(AnswerExplain) Then
                        Log.Record(Log.LogType.ManageExam, "แก้ไขคำอธิบายคำตอบเป็น """ & _DB.CleanString(AnswerExplain) & """(QuestionId=(" & ObjAnswerExplainSet(0).ToString().Trim() & ")", True)
                        UpdateStatus = 1
                    End If
                Next
            ...
            If UpdateStatus = 0 Then
                Log.Record(Log.LogType.ManageExam, "กดบันทึกปิดหน้าจอไปโดยไม่ได้แก้ไขอะไร (QuestionId=" & ObjQuestionSet(0).ToString().Trim() & ")", True)
            End If
    ...
    <Services.WebMethod()>
    Public Shared Function MoveExam(QuestionId As String, QuestionSetId As String, QuestionCategoryId As String, BookId As String) As String

        Try
            Dim sql As String = "select * from tblChangeBook where QuestionId = '" & QuestionId & "' and IsActive = 1;"
            Dim _DB As New ClassConnectSql()
            Dim dt As DataTable = _DB.getdata(sql)

            If dt.Rows.Count > 0 Then
                sql = "Update tblChangeBook set isActive = '0' where QuestionId = '" & QuestionId & "';"
                _DB.Execute(sql)
            End If

            sql = "insert into tblChangeBook(BookGroupId,QCategoryId,QSetId,QuestionId,IsActive,LastUpdate)values('" & BookId & "','" & QuestionCategoryId & "','" & QuestionSetId & "','" & QuestionId & "','1',dbo.GetThaiDate())"
            _DB.Execute(sql)
...
#Region "Other"
    ...
    Private Sub ddlBook_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlBook.SelectedIndexChanged
        BindQuestionCategory(ddlBook.SelectedValue.ToString)
    End Sub

    Private Sub ddlCategory_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlCategory.SelectedIndexChanged
        BindQuestionSet(ddlCategory.SelectedValue.ToString)
    End Sub

    Private Sub BindQuestionCategory(BookGroupId As String, Optional QuestionCatgoryId As String = "0")
        Dim Sql = "Select QCategory_Id,QCategory_Name from tblQuestionCategory where Book_Id = '" & BookGroupId & "' and IsActive = 1 order by QCategory_No"
        Dim dtQuestionCategory As DataTable
        dtQuestionCategory = _DB.getdata(Sql)
        ddlCategory.DataValueField = "QCategory_Id"
        ddlCategory.DataTextField = "QCategory_Name"
        ddlCategory.DataSource = dtQuestionCategory
        ddlCategory.DataBind()
        If QuestionCatgoryId <> "0" Then
            ddlCategory.SelectedValue = QuestionCatgoryId
        End If
    End Sub

    Private Sub BindQuestionSet(QuestionCategoryId As String, Optional QuestionSetId As String = "0")

        Dim Sql = "Select QSet_Id,QSet_Name from tblQuestionset where QCategory_Id = '" & QuestionCategoryId & "' and IsActive = 1 order by QSet_No"
        Dim dtQuestionSet As DataTable
        dtQuestionSet = _DB.getdata(Sql)
        ddlQuestionSet.DataValueField = "Qset_Id"
        ddlQuestionSet.DataTextField = "Qset_Name"
        ddlQuestionSet.DataSource = dtQuestionSet
        ddlQuestionSet.DataBind()

        If QuestionSetId <> "0" Then
            ddlQuestionSet.SelectedValue = QuestionSetId
        End If
    End Sub
