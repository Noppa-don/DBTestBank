...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ClsLog.Record("LoginPage_Load")
        'เช็คว่าจะใช้ stylesheet ตัวไหน
        'createLinkCss()
            ...
            If ClsLanguage.Chklang() = True Then
                Try
                    ...
                    If ClsKNSession.IsMaxONet Then
                        'เช็คว่า device นี้เคยลงทะเบียนมั้ย ถ้าไม่เคยก็ไปหน้าลงทะเบียน
                        'ถ้าเคยลงทะเบียน ไปหน้าเลือกวิชา (ถ้าเลือกแล้วจะ Redirect ไปหน้าแรกของ MaxOnet เอง)
    ...
    '        createLinkCss() 'เช็คว่าจะใช้ stylesheet ตัวไหน
    '        If Not IsPostBack Then
    '            ' txtemty.Visible = False
    '            txtvalidate.Visible = False
    ...
    '#If F5 = "1" Then
    '                KNConfigData.AppendF5DebugSession("StartDebug")
    '#End If
    '                Try
    '                    'ดึงค่า Config ทั้งหมดขึ้น Application
    '                    KNConfigData.LoadData()
    ...
    '                    Label1.Text = HttpContext.Current.Application("runmode").ToString

    '                Catch ex As Exception When Not TypeOf ex Is Threading.ThreadAbortException

    '#If F5 = "1" Then
    '                    KNConfigData.AppendF5DebugSession("EndDebug")
    '                    Elmah.ErrorSignal.FromCurrentContext().Raise(New Exception(Session("F5Debug").ToString()))
    '#End If
    ...
    '#If F5 <> "1" Then
    '            'เช็คเพื่อซ่อน txtbox SchoolCode
    '            ByPassSchoolCode()
    '            'HttpContext.Current.Application("runmode") = "standalonenotablet"
    '            'HttpContext.Current.Application("runmode") = "full"
    '            ' HttpContext.Current.Application("NeedCheckmark") = False

    '            'HttpContext.Current.Application("NeedEditButton") = True
    '            'HttpContext.Current.Application("NeedAddNewQCatAndQsetButton") = True
    '            'HttpContext.Current.Application("NeedAddNewQuestionButton") = True
    '            'HttpContext.Current.Application("NeedEditQuestionCategory") = True
    '#Else
    '            If Not HttpContext.Current.Application("runmode") = "twotests" Then
    '                ByPassSchoolCode()
    '            End If
    ...
    '            ' HttpContext.Current.Application("runmode") = "full"
    '            ' HttpContext.Current.Application("NeedEditButton") = True
    '            ' Application("NeedAddNewQCatAndQsetButton") = True
    '            'Application("NeedEditQuestionCategory") = True
    '            'HttpContext.Current.Application("EnableUserSubjectClass") = ""
    '#End If
    ...
    Public Sub BtnSubmit_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BtnSubmit.Click, btnLogin.Click
        ...
        If ValidatePage() Then
            If CheckIsHaveCalendarData() Then
                Log.Record(Log.LogType.Login, txtSchoolid.Text, True)
