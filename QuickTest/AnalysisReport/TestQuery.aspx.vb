Public Class TestQuery
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim SelectDate As DateTime = "3/11/2556"

        If Not Page.IsPostBack Then
            'ProcessTmpSchool(SchoolId, SelectDate)
            ProcessTmpStudent(SchoolId, SelectDate)
        End If
...
#Region "School Section"
    ...
    Private Sub ProcessTmpSchool(ByVal SchoolId As String, ByVal SelectedDate As DateTime)
    ...
    Private Function GetTotalBrokenDevice(ByVal Schoolid As String, ByVal Month As String, ByVal Year As String, ByRef ObjDb As ClassConnectSql)
        ...
        sql = " select COUNT(*) from t360_tblNetwork where DATEDIFF(DAY,CAST('" & Year & "-" & (Month + 1) & "-1' as datetime),Network_LastDate) > 7 and School_Code = '" & Schoolid & "' "
...
#Region "Student Section"
    ...
    Private Sub ProcessTmpStudent(ByVal SchoolId As String, ByVal SelectedDate As DateTime)
        Dim Month As String = SelectedDate.Month()
        Dim Year As String = CleanFormatCalendar(SelectedDate.Year())
    ...
    Private Sub InsertTmpStudentReportSendHomework(ByVal StudentId As String, ByVal Month As String, ByVal Year As String)
        ...
        HomeworkCompleteAndBeforeDeadline = _DB.ExecuteScalar(sql)
        ...
        sql = " SELECT COUNT(*) FROM tblModule INNER JOIN tblModuleAssignment ON tblModule.Module_Id = tblModuleAssignment.Module_Id INNER JOIN " & _
              ...
              " (DATEPART(YEAR, tblModuleAssignment.End_Date) = '" & Year & "') AND (dbo.tblModuleDetailCompletion.Module_Status = 2 OR dbo.tblModuleDetailCompletion.Module_Status = 3) " & _
              " AND dbo.tblModuleDetailCompletion.IsActive = 1 AND dbo.tblModule.IsActive = 1 "
        HomeworkNotComplete = _DB.ExecuteScalar(sql)

        'ไม่ส่ง
        sql = " SELECT COUNT(*) FROM tblModule INNER JOIN tblModuleAssignment ON tblModule.Module_Id = tblModuleAssignment.Module_Id INNER JOIN " & _
              " tblModuleDetailCompletion ON tblModuleAssignment.MA_Id = tblModuleDetailCompletion.MA_Id WHERE " & _
              " (tblModuleDetailCompletion.Student_Id = '" & StudentId & "') AND (DATEPART(MONTH, tblModuleAssignment.End_Date) = '" & Month & "') AND " & _
              " (DATEPART(YEAR, tblModuleAssignment.End_Date) = '" & Year & "' AND dbo.tblModuleDetailCompletion.IsActive = 1 AND dbo.tblModule.IsActive = 1 " & _
              " AND dbo.tblModuleDetailCompletion.TimeExitedByUser IS NULL "
        HomeworkNotSend = _DB.ExecuteScalar(sql)
