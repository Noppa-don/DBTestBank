...
Public Class SelectSession
    ...
    Private Sub CreateDataByArrData(ByVal ArrData As ArrayList)
        ...
        Else
            ...
            Dim ArrStrColumn As Array = {"PK", "ลำดับ", "วัน-เวลา", "ชุดข้อสอบ", "ห้อง", "ห้อง Lab", "หน้าจอ"}
            ...
            For Each EachObj In ArrData 'วน Add ข้อมูลลง Datatable ทีละ Cls จนครบตามจำนวน Array ของ Application
                ...
                timeString = ClsTimeAgo.GetPhaseTimeAgo(CType(ClsSessInfo.TimeStamp, DateTime))
                ...
                dt.Rows.Add(Pk, Index, timeString, TestSetName, ClassName, LabName, ScreenName)
                ClsSessInfo = Nothing
                Index += 1
            Next
    ...
    Private Sub CreateDataOnLightbox(ArrData As ArrayList)
        Dim dt As New DataTable
        Dim ArrStrColumn As Array = {"PK", "ลำดับ", "กิจกรรม", "วัน-เวลา"}
        Dim BoundColumn As New Telerik.Web.UI.GridBoundColumn
        ...
        For Each EachObj In ArrData 'วน Add ข้อมูลลง Datatable ทีละ Cls จนครบตามจำนวน Array ของ Application
            ...
            dt.Rows.Add(Pk, txtHmtl, ActivityName.ToString(), ActivityTime.ToString())
            ClsSessInfo = Nothing
            ActivityName.Clear()
            ActivityTime.Clear()
            Index += 1
